const AssignedCourseEnrollmentCard = ({name, id, coursetype, coursecategory, duration, trainingmode, trainingtype, course, selected, onclick,numberOfEnrollees }) => {
    ...
    return(
        <div className={`group w-full p-3 grid grid-cols-[auto_3.75rem] border border-divider rounded-md font-text shadow-md hover:cursor-pointer hover:bg-primary hover:text-white transition-all ease-in-out ${course === name ? 'bg-primary text-white' : 'bg-white text-primary'}`}
            onClick={onclick}>
            {/* Badge */}
